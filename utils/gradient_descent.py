...
def regression_gradient_descent(feature_matrix: pd.DataFrame, output: pd.Series, initial_weights: np.array, step_size: float, tolerance: float, round_res=True) -> np.array:
	converged = False 
	...
	while not converged:
		...
		if round_res:
			errarr = np.round(prognoza) - output
		else:
